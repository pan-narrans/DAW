...
// Reutiliza el c�digo de ArgPersona que devuelve un Stream de Arguments
...
// En java la solución funcional es compleja por dos motivos
		...
		.map(argumentos -> Arrays.asList(argumentos)) // Lo convierto a lista para poder a�adirle un nuevo argumento: el id aleatorio
		.map(lista -> new ArrayList<>(lista)) // �apa de java: tengo que crear una nueva lista porque la anterior no es ampliable
		.map(listaNueva -> Stream.concat(Aleatorio.getIntList(-100, 100, 1).stream(),listaNueva.stream()).collect(Collectors.toList())) // Otra �apa: este es el equivalente a lista1 + lista2
