...
*/		  		
    ...
    if (nChar > nCharMax) throw new IllegalArgumentException("Exception: nChar > " + nCharMax);
		...
		getIntList(97, 122, nChar) // Genera una lista de nChar enteros con los ascii de las minúsculas
			.stream() // iterable
			.map(entero -> Character.toString(entero)) // convierte el int al caracter ascii correspondiente
			.forEach(caracter -> resultado.append(caracter)); // junta todos los caracteres en una cadena de longitud nChar
