...
public class Clase {
  ...
  /**
   * Es un m�todo que itera por todos los campos de una clase invocando los
   * m�todos "getCampo()" para obtener los valores
   * Forma un texto donde cada l�nea es una concatenaci�n del nombre del campo y
   * su valor
   ...
   * @return String Una l�nea para cada campo: Descripcion + Valor. Primera nombre
   *         clase
  ...
  public static String imprimeClase(Object objeto) throws IllegalArgumentException {
    ...
    // Primera l�nea nombre de la clase
    String nClase = objeto.getClass().getName(); // Nombre de la clase (con el nombre del paquete y un punto por
                                                 // delante)
    nClase = nClase.substring(nClase.indexOf(".") + 1); // Se queda con la parte de la derecha despu�s del punto
    nClase = nClase.toUpperCase(); // En may�sculas
  ...
  /**
   * Es un m�todo que compara dos objetos de la misma clase: original y copia
   * S�lo funciona si los nombres de los campos se definen en min�sculas, y para
  ...
  public static Map<String, Object> getValores(Object objeto) throws IllegalArgumentException {
    ...
    for (String nCampo : nCampos) {
      ...
      } // Captura el m�todo con nombre getCampo
      ...
      } // Invoca el m�todo getCampo sobre ese objeto y recupera el valor del campo.
  ...
  /**
   * Recupera todos los campos de una clase: propios y heredados, p�blicos y
   * privados
   * Es necesario hacerlo as� porque el m�todo Class.getDeclaredFields() no
  ...
  public static List<Field> getAllFields(Object objeto) throws IllegalArgumentException {
    ...
    for (Class<?> c = objeto.getClass(); c != null; c = c.getSuperclass()) { // Recorre la jeraqu�a de la clase
      fields.addAll(Arrays.asList(c.getDeclaredFields()));
    } // Va a�adiendo los campos

    return fields;
  }

  /**
   * Recupera todos los m�todos de una clase: propios y heredados, p�blicos y
   * privados
   * Es necesario hacerlo as� porque el m�todo Class.getDeclaredMethods() no
   * devuelve los heredados
   * 
   * @author bfrancoc@gmail.com
   * @version 1.0
   * @since 2020-03-25
   * @param objeto Objeto de la clase de la que se quieren obtener los campos
   * @return List Lista con los m�todos (tipo Method)
   * @throws IllegalArgumentException Exception si clase = nulo
   */
  ...
  /**
   * Imprime solo los nombres de los campos que contienen cierto texto. Si est� en
   * blanco, todos los campos
   * 
   * @author bfrancoc@gmail.com
   * @version 1.0
   * @param objeto Instancia que se quiere imprimir
   * @param texto  S�lo imprime los campos que tengan ese texto. Si es "" imprime
   *               todos
   * @throws IllegalArgumentException Exception si clase = nulo
   */

  // No hay return porque se trata de hacer un syso de los campos: devuelve void
  // El pr�ximo m�todo devuelve la ristra de nombres (String)
  ...
  public static void imprimeNombreCampos(Object objeto, String texto) throws IllegalArgumentException {
    ...
    campos
        ...
        .map(Field::getName) // Toma el nombre de los Field (method reference)
        .filter(nCampo -> nCampo.contains(texto)) // Filtra los campos que tengan el texto (par�metro de entrada)
        .forEach(campo -> System.out.println(Cadena.primeraMayuscula(campo)));
  } // Para cada campo imprime una l�nea con su nombre (primera en may�scula)
  ...
  public static String getNombreCamposString(Object objeto) throws IllegalArgumentException {
    ...
    campos
        ...
        .map(Cadena::primeraMayuscula) // Pone la primera en may�scula
  ...
  public static void imprimeNombreMetodos(Object objeto, String texto) throws IllegalArgumentException {
    ...
    metodos
        ...
        .filter(nMetodo -> nMetodo.contains(texto)) // Filtra los metodos que tengan el texto (par�metro de entrada)
        .forEach(metodo -> System.out.println(Cadena.primeraMayuscula(metodo)));
  } // Para cada metodo imprime una l�nea con su nombre (primera en may�scula)
  ...
  public static void main(String[] args) {
     ...
     * 
     * Factura factura = new
     * Factura(1,"19/03/2022","12755355S","06023717V","Rotura de Tuber�a",1000,0.21,
     * 0.20);
