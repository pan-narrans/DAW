...
public class Clase {
   ...
   * métodos "getCampo()" para obtener los valores Forma un texto donde cada
   * línea es una concatenación del nombre del campo y su valor La primera línea
   * es el nombre de la clase Hay que hacerlo indirectamente, invocando los
   * getCampos(), porque se asume que los campos son private y no se puede
   * acceder a su valor de otra forma Sólo funciona si los nombres de los campos
   * se definen en minúsculas, y para cada campo hay un getCampo en camelCase
   * 
   * @author Benito Franco
   * @version 1.1
   * @since 2020-03-21
   * @param objeto Instancia de la clase que se quiere imprimir
   * @return String Una línea para cada campo: Descripción + Valor. Primera
   *         nombre clase
   * @throws IllegalArgumentException Exception si objeto = nulo
   */

  public static String imprimeClase(Object objeto)
    ...
    // Array con los campos (Field) de la clase
    campos = getAllFields(objeto.getClass());

    for (Field campo : campos) { // Itera por todos los campos = forEach
      nCampo = campo.getName(); // Toma el nombre del campo
      nCampo = Cadena.primeraMayuscula(nCampo); // Pone la primera en mayúscula
      ...
      } // Invoca el método getCampo y recupera el valor del campo.
   ...
   * privados Es necesario hacerlo as porque el método Class.getDeclaredFields()
   * no devuelve los heredados
   * 
   * @author Benito Franco
   * @version 1.0
   * @since 2020-03-21
  ...
  public static List<Field> getAllFields(Class<?> type)
    ...
    // Recorre la jerarquía de la clase
    for (Class<?> c = type; c != null; c = c.getSuperclass()) {
      fields.addAll(Arrays.asList(c.getDeclaredFields()));
    } // Va añadiendo los campos
